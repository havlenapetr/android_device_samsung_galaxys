...
#define MEM_DRIVER_NAME		"/dev/s3c-mem"
...
typedef struct {
	...
	enc_dec_type_t	dec_type;
	jpeg_out_mode      out_format;
	UINT32	width;
	UINT32	height;
	UINT32	data_size;
	UINT32	file_size;
...
typedef struct {
	sample_mode_t	sample_mode;
	enc_dec_type_t	enc_type;
	jpeg_in_mode      in_format;
...
typedef struct ___JPGLIB {
	int  jpg_fd;
	int  cmm_fd;
	jpg_args  args;
	UINT8	thumbnail_flag;
	exif_file_info_t *exif_info;
} jpg_lib;
...
static jpeg_error_type make_thumb_image(int dev_fd);
static jpeg_error_type mcu_check(sample_mode_t sample_mode, UINT32 width, UINT32 height);
static jpeg_error_type make_exif_file(char *exif_out, UINT *total_len);
static void scale_down_yuv422(char *src_buf, INT32 src_width, INT32 src_height,
			      char *dst_buf, INT32 dst_width, INT32 dst_height);


//////////////////////////////////////////////////////////////////////////////////////
// Variables																		//
//////////////////////////////////////////////////////////////////////////////////////
jpg_lib  *jpg_ctx = NULL;
...
static void init_decode_param(void)
{
	jpg_ctx = (jpg_lib *)malloc(sizeof(jpg_lib));
	memset(jpg_ctx, 0x00, sizeof(jpg_lib));

	jpg_ctx->args.dec_param = (jpg_dec_proc_param *)malloc(sizeof(jpg_dec_proc_param));
	memset(jpg_ctx->args.dec_param, 0x00, sizeof(jpg_dec_proc_param));
...
int SsbSipJPEGDecodeInit(void)
{
	...
	if (jpg_ctx->jpg_fd < 0) {
		LOGE("SsbSipJPEGDecodeInit - JPEG Driver open failed\n");
		return -1;
	}
	...
	if (jpg_ctx->args.mmapped_addr == NULL) {
		LOGE("SsbSipJPEGDecodeInit - JPEG mmap failed\n");
		return -1;
	}

	LOGV("SsbSipJPEGDecodeInit - jpg_ctx->args.out_buf  0x%x, jpg_ctx->args.phy_out_buf : 0x%x\n", jpg_ctx->args.out_buf , jpg_ctx->args.phy_out_buf);
	LOGV("SsbSipJPEGDecodeInit - init decode handle : %d\n", jpg_ctx->jpg_fd);
...
static void init_encode_param(void)
{
	...
	if (jpg_ctx == NULL) {
		LOGE("init_encode_param - JPEG LIB init failed\n");
		return;
	}

	memset(jpg_ctx, 0x00, sizeof(jpg_lib));

	jpg_ctx->args.enc_param = (jpg_enc_proc_param *)malloc(sizeof(jpg_enc_proc_param));

	if (jpg_ctx->args.enc_param == NULL) {
		LOGE("init_encode_param - API :: Out of Memory\n");
		return;
	}

	memset(jpg_ctx->args.enc_param, 0x00, sizeof(jpg_enc_proc_param));
	jpg_ctx->args.enc_param->file_size = MAX_FILE_SIZE;

	jpg_ctx->args.thumb_enc_param = (jpg_enc_proc_param *)malloc(sizeof(jpg_enc_proc_param));

	if (jpg_ctx->args.thumb_enc_param == NULL) {
		LOGE("init_encode_param - API :: Out of Memory\n");
		return;
	}

	memset(jpg_ctx->args.thumb_enc_param, 0x00, sizeof(jpg_enc_proc_param));
	jpg_ctx->args.thumb_enc_param->data_size = MAX_FILE_THUMB_SIZE;

	jpg_ctx->args.enc_param->sample_mode = JPG_420;
	jpg_ctx->args.enc_param->enc_type = JPG_MAIN;
	jpg_ctx->args.thumb_enc_param->sample_mode = JPG_420;
	jpg_ctx->args.thumb_enc_param->enc_type = JPG_THUMBNAIL;
	jpg_ctx->thumbnail_flag = FALSE;
}
...
{
	init_encode_param();
	jpg_ctx->jpg_fd = open(JPG_DRIVER_NAME, O_RDWR/*|O_NONBLOCK O_NDELAY*/);

	if (jpg_ctx->jpg_fd < 0) {
		LOGE("SsbSipJPEGEncodeInit - JPEG Driver open failed\n");
		return -1;
	}
...
{
	jpg_args *pArgs;
	LOGV("SsbSipJPEGDecodeExe - jpg_ctx->args.dec_param->file_size: %d\n", jpg_ctx->args.dec_param->file_size);
	pArgs = &(jpg_ctx->args);

	if (jpg_ctx->args.dec_param->dec_type == JPG_MAIN) {
		ioctl(jpg_ctx->jpg_fd, IOCTL_JPG_DECODE, pArgs);
		LOGV("SsbSipJPEGDecodeExe - dec_param->width : %d dec_param->height : %d\n", jpg_ctx->args.dec_param->width, jpg_ctx->args.dec_param->height);
		LOGV("SsbSipJPEGDecodeExe - streamSize : %d\n", jpg_ctx->args.dec_param->data_size);
	} else {
...
static jpeg_error_type make_thumb_image(int dev_fd)
{
	...
	if ((result = mcu_check(jpg_ctx->args.thumb_enc_param->sample_mode, jpg_ctx->args.thumb_enc_param->width, jpg_ctx->args.thumb_enc_param->height)) != JPEG_OK) {
		LOGE("make_thumb_image - thumbnail width/height doesn't match with MCU\r\n");
		return JPEG_FAIL;
	}

	// encode thumbnail image
	jpg_ctx->args.enc_param->enc_type = JPG_THUMBNAIL;
	ioctl(jpg_ctx->jpg_fd, IOCTL_JPG_ENCODE, &(jpg_ctx->args));

	if (jpg_ctx->args.thumb_enc_param->file_size > MAX_FILE_THUMB_SIZE) {
		LOGE("make_thumb_image - thumbnail data is too big\n");
		return JPEG_FAIL;
	}

	jpg_ctx->args.out_thumb_buf = (char *)ioctl(jpg_ctx->jpg_fd, IOCTL_JPG_GET_THUMB_STRBUF, jpg_ctx->args.mmapped_addr);

	LOGV("make_thumb_image - thumbFilesize : %d\n", jpg_ctx->args.thumb_enc_param->file_size);
	LOGV("make_thumb_image - jpg_ctx->args.out_thumb_buf(addr : 0x%x)\n", jpg_ctx->args.out_thumb_buf);
			      ...
			      char *dst_buf, INT32 dst_width, INT32 dst_height)
{
	...
	INT32	step_x, step_y;
	INT32	x, y, src_y_start_pos, dst_pos, src_pos;
	...
	{
	    src_y_start_pos = (y*step_y * (src_width*2));
...
jpeg_error_type SsbSipJPEGEncodeExe(int dev_fd, exif_file_info_t *Exif, jpeg_scaler scaler)
{
	...
	jpeg_error_type 	result;
	jpg_args *pArgs;

	pArgs = &(jpg_ctx->args);

	// check MCU validation width & height & sampling mode
	if ((result = mcu_check(jpg_ctx->args.enc_param->sample_mode, jpg_ctx->args.enc_param->width, jpg_ctx->args.enc_param->height)) != JPEG_OK) {
		LOGE("SsbSipJPEGEncodeExe - width/height doesn't match with MCU\r\n");
		return JPEG_FAIL;
	}

	// check validation of input datasize
	LOGV("SsbSipJPEGEncodeExe - encode width : %d, encode height : %d\n", jpg_ctx->args.enc_param->width , jpg_ctx->args.enc_param->height);

	if (jpg_ctx->args.enc_param->width*jpg_ctx->args.enc_param->height*2 != jpg_ctx->args.enc_param->data_size) {
		LOGE("SsbSipJPEGEncodeExe - data size and width/height doesn't match\r\n");
		return JPEG_FAIL;
	}

	// scale down YUV data for thumbnail encoding
	if ((Exif != NULL) && (jpg_ctx->thumbnail_flag == TRUE)) {

		jpg_ctx->args.thumb_enc_param->data_size = jpg_ctx->args.thumb_enc_param->width * jpg_ctx->args.thumb_enc_param->height * 2;
		LOGV("SsbSipJPEGEncodeExe - encode width: %d, height : %d\n", jpg_ctx->args.enc_param->width, jpg_ctx->args.enc_param->height);
		LOGV("SsbSipJPEGEncodeExe - Thumbnail data size : %d\n", jpg_ctx->args.thumb_enc_param->data_size);
		jpg_ctx->args.in_thumb_buf = (char *)SsbSipJPEGGetThumbEncInBuf(jpg_ctx->jpg_fd, jpg_ctx->args.thumb_enc_param->data_size);

		// If post-processor was loaded, H/W post-processor will be used for scaling
		if (scaler == JPEG_USE_HW_SCALER) {
			LOGV("SsbSipJPEGEncodeExe - H/W post-processor is working1\n");

			pp_fd = open(POST_DRIVER_NAME, O_RDWR);

			if (pp_fd < 0) {
				scale_down_yuv422(jpg_ctx->args.in_buf, jpg_ctx->args.enc_param->width, jpg_ctx->args.enc_param->height, 	\
						  jpg_ctx->args.in_thumb_buf, jpg_ctx->args.thumb_enc_param->width, jpg_ctx->args.thumb_enc_param->height);

				goto Lable;
			}

			pp_param.src_full_width		= jpg_ctx->args.enc_param->width;
			pp_param.src_full_height	= jpg_ctx->args.enc_param->height;
			...
			pp_param.dst_full_width		= jpg_ctx->args.thumb_enc_param->width;
			pp_param.dst_full_height	= jpg_ctx->args.thumb_enc_param->height;
			pp_param.dst_width			= pp_param.dst_full_width;
			pp_param.dst_height			= pp_param.dst_full_height;
			pp_param.dst_color_space	= YCBYCR;
			pp_param.out_path			= DMA_ONESHOT;
			ioctl(pp_fd, S3C_PP_SET_PARAMS, &pp_param);

			pp_param.src_buf_addr_phy	= ioctl(dev_fd, IOCTL_JPG_GET_PHY_FRMBUF);
			ioctl(pp_fd, S3C_PP_SET_SRC_BUF_ADDR_PHY, &pp_param);
			pp_param.dst_buf_addr_phy	= ioctl(dev_fd, IOCTL_JPG_GET_PHY_THUMB_FRMBUF);
			ioctl(pp_fd, S3C_PP_SET_DST_BUF_ADDR_PHY, &pp_param);

			ioctl(pp_fd, S3C_PP_START);
		} else {
			LOGV("SsbSipJPEGEncodeExe - S/W scaler is working\n");
			LOGV("SsbSipJPEGEncodeExe - 0x%08x %d: %d: \n", jpg_ctx->args.in_buf, jpg_ctx->args.enc_param->width, jpg_ctx->args.enc_param->height);
			LOGV("SsbSipJPEGEncodeExe - 0x%08x %d: %d: \n", jpg_ctx->args.in_thumb_buf, jpg_ctx->args.thumb_enc_param->width, jpg_ctx->args.thumb_enc_param->height);
...
Lable:
	// encode main image
	ioctl(jpg_ctx->jpg_fd, IOCTL_JPG_ENCODE, pArgs);

	// if user want to make Exif file
	if (Exif) {
		jpg_ctx->exif_info = Exif;

		// encode thumbnail image
		if (jpg_ctx->thumbnail_flag) {
			if ((ret = make_thumb_image(jpg_ctx->jpg_fd)) != JPEG_OK)
				return JPEG_FAIL;

			bufSize = EXIF_FILE_SIZE + jpg_ctx->args.thumb_enc_param->file_size;
		} else
			bufSize = EXIF_FILE_SIZE;

		if (jpg_ctx->args.enc_param->file_size + bufSize > MAX_FILE_SIZE) {
			LOGE("SsbSipJPEGEncodeExe - Exif Error - out of buffer\n");
			return JPEG_FAIL;
		}
		...
		if ((result = make_exif_file(ExifBuf, &ExifLen)) != JPEG_OK) {
			free(ExifBuf);
			return JPEG_FAIL;
		}

		LOGV("SsbSipJPEGEncodeExe - ExifLen : %d jpg_ctx->enc_param->filesize : %d\n", ExifLen, jpg_ctx->args.enc_param->file_size);
		jpg_ctx->args.out_buf = (char *)ioctl(jpg_ctx->jpg_fd, IOCTL_JPG_GET_STRBUF, jpg_ctx->args.mmapped_addr);

		// merge Exif file with encoded JPEG file header
		memmove(&jpg_ctx->args.out_buf[ExifLen+2], &jpg_ctx->args.out_buf[2], jpg_ctx->args.enc_param->file_size - 2);
		memcpy(&jpg_ctx->args.out_buf[2], ExifBuf, ExifLen);
		jpg_ctx->args.enc_param->file_size += ExifLen;
		jpg_ctx->exif_info = NULL;
...
void *SsbSipJPEGGetDecodeInBuf(int dev_fd, long size)
{
	...
	if (size < 0 || size > MAX_FILE_SIZE) {
		LOGE("SsbSipJPEGGetDecodeInBuf - Invalid Decoder input buffer size\r\n");
		return NULL;
	}
	...
	return (void *)(jpg_ctx->args.in_buf);
}
...
void *SsbSipJPEGGetDecodeOutBuf(int dev_fd, long *size)
{
	...
	*size = jpg_ctx->args.dec_param->data_size;
	...
	LOGV("SsbSipJPEGGetDecodeOutBuf - DecodeOutBuf : 0x%x  size : %d\n", jpg_ctx->args.out_buf, jpg_ctx->args.dec_param->data_size);
...
void *SsbSipJPEGGetEncodeInBuf(int dev_fd, long size)
{
	...
	jpg_ctx->args.enc_param->data_size = size;
	jpg_ctx->args.in_buf = (char *)ioctl(jpg_ctx->jpg_fd, IOCTL_JPG_GET_FRMBUF, jpg_ctx->args.mmapped_addr);

	LOGV("SsbSipJPEGEncodeInBuf - EncodeInBuf : 0x%x  size :%d\n", jpg_ctx->args.in_buf, jpg_ctx->args.enc_param->data_size);
	return (void *)(jpg_ctx->args.in_buf);

}

/*----------------------------------------------------------------------------
*Function: SsbSipJPEGSetEncodeInBuf

*Parameters: dev_fd: JPEG device descriptor
	     phys_frmbuf : physical address of the frame buffer
             *size : output stream(YUV/RGB) size
*Implementation Notes: set encoder input buffer physical address
-----------------------------------------------------------------------------*/
void SsbSipJPEGSetEncodeInBuf(int dev_fd, UINT32 phys_frmbuf, long size)
{
	jpg_ctx->args.enc_param->data_size = size;
	jpg_ctx->args.enc_param->set_framebuf = 1;
	ioctl(jpg_ctx->jpg_fd, IOCTL_JPG_SET_FRMBUF, phys_frmbuf);
}

int SetMapAddr(unsigned char *addr)
{
	if(addr == NULL)
		return JPEG_FAIL;

	jpg_ctx->args.mmapped_addr = (char *)addr;
	LOGV("SetMapAddr - mapped jpg address = %0x",jpg_ctx->args.mmapped_addr);
...
void *SsbSipJPEGGetEncodeOutBuf(int dev_fd, long *size)
{
	...
	jpg_ctx->args.out_buf = (char *)ioctl(jpg_ctx->jpg_fd, IOCTL_JPG_GET_STRBUF, jpg_ctx->args.mmapped_addr);
	*size = jpg_ctx->args.enc_param->file_size;

	LOGV("SsbSipJPEGGetEncodeOutBuf - jpg_ctx->args.out_buf(addr : 0x%x  size : %d)\n", jpg_ctx->args.out_buf, *size);
	return (void *)(jpg_ctx->args.out_buf);
}
...
void *SsbSipJPEGGetThumbEncInBuf(int dev_fd, long size)
{
	...
	jpg_ctx->args.in_thumb_buf = (char *)ioctl(dev_fd, IOCTL_JPG_GET_THUMB_FRMBUF, jpg_ctx->args.mmapped_addr);

	return (void *)(jpg_ctx->args.in_thumb_buf);
...
{
	jpeg_error_type result = JPEG_OK;
	LOGV("SsbSipJPEGGetConfig - Config type %d, value : %d\n",type,*value);
	switch (type) {
	...
	case JPEG_GET_SAMPING_MODE:
		*value = (INT32)jpg_ctx->args.dec_param->sample_mode;
		break;
	default :
		LOGE("SsbSipJPEGGetConfig - Invalid Config type\n");
		result = JPEG_FAIL;
	}
...
jpeg_error_type SsbSipJPEGSetConfig(jpeg_conf type, INT32 value)
{
	...
	LOGV("SsbSipJPEGSetConfig - SsbSipJPEGSetConfig(%d : %d)\n", type, value);

	switch (type) {
	case JPEG_SET_DECODE_OUT_FORMAT: {
		if (value != YCBCR_420 && value != YCBCR_422) {
			LOGE("SsbSipJPEGSetConfig - API :: Invalid decode output format\r\n");
			result = JPEG_FAIL;
		} else {
	...
	case JPEG_SET_ENCODE_WIDTH: {
		if (value < 0 || value > MAX_JPG_WIDTH) {
			LOGE("SsbSipJPEGSetConfig - Invalid width\r\n");
			result = JPEG_FAIL;
		} else
	...
	case JPEG_SET_ENCODE_HEIGHT: {
		if (value < 0 || value > MAX_JPG_HEIGHT) {
			LOGE("SsbSipJPEGSetConfig - Invalid height\n");
			result = JPEG_FAIL;
		} else
	...
	case JPEG_SET_ENCODE_QUALITY: {
		if (value < JPG_QUALITY_LEVEL_1 || value > JPG_QUALITY_LEVEL_4) {
			LOGE("SsbSipJPEGSetConfig - Invalid Quality value\n");
			result = JPEG_FAIL;
		} else
	...
	case JPEG_SET_ENCODE_THUMBNAIL: {
		if (value != TRUE && value != FALSE) {
			LOGE("SsbSipJPEGSetConfig - Invalid thumbnail_flag\n");
			result = JPEG_FAIL;
		} else
	...
	case JPEG_SET_ENCODE_IN_FORMAT: {
		if (value != JPG_MODESEL_YCBCR && value != JPG_MODESEL_RGB) {
			LOGE("SsbSipJPEGSetConfig - Invalid informat\n");
			result = JPEG_FAIL;
		} else {
	...
	case JPEG_SET_SAMPING_MODE: {
		if (value != JPG_420 && value != JPG_422) {
			LOGE("SsbSipJPEGSetConfig - Invalid sample_mode\r\n");
			result = JPEG_FAIL;
		} else {
	...
	case JPEG_SET_THUMBNAIL_WIDTH: {
		if (value < 0 || value > MAX_JPG_THUMBNAIL_WIDTH) {
			LOGE("SsbSipJPEGSetConfig - Invalid thumbWidth\r\n");
			result = JPEG_FAIL;
		} else
	...
	case JPEG_SET_THUMBNAIL_HEIGHT: {
		if (value < 0 || value > MAX_JPG_THUMBNAIL_HEIGHT) {
			LOGE("SsbSipJPEGSetConfig - Invalid thumbHeight\r\n");
			result = JPEG_FAIL;
		} else
	...
	//for the future work....
	default :
		LOGE("SsbSipJPEGSetConfig - Invalid Config type\n");
		result = JPEG_FAIL;
	}
...
jpeg_error_type SsbSipJPEGDecodeDeInit(int dev_fd)
{
	...
	LOGV("SsbSipJPEGDecodeDeInit - dev: %d\n", dev_fd);
...
jpeg_error_type SsbSipJPEGEncodeDeInit(int dev_fd)
{
	...
	if (jpg_ctx->args.thumb_enc_param != NULL)
		free(jpg_ctx->args.thumb_enc_param);

	if (jpg_ctx->exif_info != NULL)
		free(jpg_ctx->exif_info);
...
static jpeg_error_type make_exif_file(char *exif_out, UINT *total_len)
{
	...
	LOGV("make_exif_file - makeExif\n");
	ExifInitialCount = tempExif;
	//for APP1 Marker(2 byte) and length(2 byte)
	tempExif += 4;
	//write an exif header
	memcpy(tempExif, exif_header, 6);
	tempExif += 6 ;

	//write a tiff header
	memcpy(tempExif, tiff_header, 8);
	...
	memcpy(make, jpg_ctx->exif_info->make, 32);
	offset = (UCHAR *) 0x200;
	santemp = (int)(offset);
...
//	strcpy(model,tpJEInfo->Model);
	memcpy(model, jpg_ctx->exif_info->Model, 32);
	santemp = (int)(offset);
	...
	Orientation[0] = jpg_ctx->exif_info->Orientation;
	...
	tempExif += 4 ;
	//write orientation mode
	Process[0] = jpg_ctx->exif_info->Process;
...
//	strcpy(Version,jCtx->ExifInfo->Version);
	memcpy(Version, jpg_ctx->exif_info->Version, 32);
	...
	memcpy(DateTime, jpg_ctx->exif_info->DateTime, 20);
...
//	strcpy(CopyRight,jCtx->ExifInfo->CopyRight);////="copyright 2006";);
	memcpy(CopyRight, jpg_ctx->exif_info->CopyRight, 32);
...
///////////////ENTRY NO 1 : EXPOSURE TIME////////////////////////
	...
	ExposureTimeNum[0] = jpg_ctx->exif_info->ExposureTimeNum;
	ExposureTimeDen[0] = jpg_ctx->exif_info->ExposureTimeDen;
	...
	FNumberNum[0] = jpg_ctx->exif_info->FNumberNum;
	FNumberDen[0] = jpg_ctx->exif_info->FNumberDen;
	...
	tempExif += 4 ;
	//write orientation mode
	ExposureProgram[0] = jpg_ctx->exif_info->ExposureProgram;
...
///////////////ENTRY NO 4 :ISOSpeedRatings ////////////////////////
	...
	BrightnessNum[0] = jpg_ctx->exif_info->BrightnessNum;
	BrightnessDen[0] = jpg_ctx->exif_info->BrightnessDen;
	...
	tempExif += 4 ;
	ExposureBiasNum[0] = jpg_ctx->exif_info->ExposureBiasNum;
	ExposureBiasDen[0] = jpg_ctx->exif_info->ExposureBiasDen;
...
///////////////ENTRY NO 7 : SUBJECT DISTANCE////////////////////////
	...
	tempExif += 4 ;
	SubjectDistanceNum[0] = jpg_ctx->exif_info->SubjectDistanceNum;
	SubjectDistanceDen[0] = jpg_ctx->exif_info->SubjectDistanceDen;
	...
	tempExif += 4 ;
	//write METERING mode
	MeteringMode[0] = jpg_ctx->exif_info->MeteringMode;
	...
	tempExif += 4 ;
	//write FLASH mode
	Flash[0] = jpg_ctx->exif_info->Flash;
	...
	tempExif += 4 ;
	FocalLengthNum[0] = jpg_ctx->exif_info->FocalLengthNum;
	FocalLengthDen[0] = jpg_ctx->exif_info->FocalLengthDen;
	...
	tempExif += 4 ;
	//write Width
	Width[0] = jpg_ctx->exif_info->Width;
	...
	tempExif += 4 ;
	//write Height
	Height[0] = jpg_ctx->exif_info->Height;
	...
	tempExif += 4 ;
	//write orientation mode
	ColorSpace [0] = jpg_ctx->exif_info->ColorSpace;
...
///////////////ENTRY NO 14 : FocalPlaneXResolution////////////////////////
	...
	tempExif += 4 ;
	FocalPlaneXResolutionNum[0] = jpg_ctx->exif_info->FocalPlaneXResolutionNum;
	FocalPlaneXResolutionDen[0] = jpg_ctx->exif_info->FocalPlaneXResolutionDen;
	...
	tempExif += 4 ;
	FocalPlaneYResolutionNum[0] = jpg_ctx->exif_info->FocalPlaneYResolutionNum;
	FocalPlaneYResolutionDen[0] = jpg_ctx->exif_info->FocalPlaneYResolutionDen;
...
///////////////ENTRY NO 16 :FocalPlaneResolutionUnit////////////////////////
	...
	FocalPlaneResolutionUnit[0] = jpg_ctx->exif_info->FocalPlaneResolutionUnit;
	...
	//write model
//	strcpy(model,tpJEInfo->Model);
	memcpy(UserComments, jpg_ctx->exif_info->UserComments, 150);
	santemp = (int)(offset);
...
///////////////ENTRY NO 18 :WHITE BALANCE////////////////////////
	...
	tempExif += 4 ;
	//write orientation mode
	WhiteBalance[0] = jpg_ctx->exif_info->WhiteBalance;
...
///////////////ENTRY NO 2 : X - RESOLUTION OF THE IMAGE////////////////////////
		...
		tempExif += 4 ;
		XResolutionNum[0] = jpg_ctx->exif_info->XResolutionNum;
		XResolutionDen[0] = jpg_ctx->exif_info->XResolutionDen;
		...
		tempExif += 4 ;
		YResolutionNum[0] = jpg_ctx->exif_info->YResolutionNum;
		YResolutionDen[0] = jpg_ctx->exif_info->YResolutionDen;
		...
		tempExif += 4 ;
		//write orientation mode
		RUnit[0] = jpg_ctx->exif_info->RUnit;
		...
		tempExif += 4 ;
		//write the  offset to the SubIFD
		santemp = jpg_ctx->args.thumb_enc_param->file_size;
		...
		memcpy(startoftiff + santemp, jpg_ctx->args.out_thumb_buf, jpg_ctx->args.thumb_enc_param->file_size);
	}
	...
	if (jpg_ctx->thumbnail_flag) {
		offset += jpg_ctx->args.thumb_enc_param->file_size;
	}
	...
	if (ExifSize > EXIF_FILE_SIZE + MAX_FILE_THUMB_SIZE - 2 || ExifSize < 0) {
		LOGE("make_exif_file - Invalid Exif size\r\n");
		tempExif = NULL;
		*total_len = 0;
		return JPEG_FAIL;
	}
	...
	*total_len = ExifSize + 2;
	LOGI("make_exif_file - total_len : %d\n", *total_len);
	return JPEG_OK;
}
